...
/**
 * Load the system prompt from configs/prompt.md
 */
function loadSystemPrompt(): string {
  const promptPath = path.join(process.cwd(), 'configs', 'prompt.md');
  return fs.readFileSync(promptPath, 'utf-8');
}

/**
 * Load the knowledge base from configs/kb.yaml
 */
function loadKnowledgeBase(): string {
  const kbPath = path.join(process.cwd(), 'configs', 'kb.yaml');
